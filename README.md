...
目前階段實作 Codex 用量監控，在私人 Discord 文字頻道維護一則用量訊息。畫面上的百分比與進度條表示**剩餘量**（100% 減去 Codex 回傳的已用百分比）。Account 1 使用真實 Codex 帳號；Account 2、3 是固定的畫面範本。程式每五分鐘透過容器內的 `codex app-server --stdio` 讀取用量，並編輯同一則訊息。
...
4. 用 `docker compose logs -f codex-ratelimite-bot` 檢查啟動結果；用 `docker compose exec codex-ratelimite-bot codex --version` 確認容器內 CLI。Discord 頻道應只有一則 `Codex Usage Monitor` 儀表板。

若憑證過期，先在 Pi 主機以相同使用者執行 `codex login`，然後執行 `docker compose restart codex-ratelimite-bot`。更新程式後執行 `docker compose up -d --build`；`data/state.json` 會讓新容器沿用原訊息及最後成功讀取的用量。服務使用 `restart: unless-stopped`，會在 Pi 或 Docker 重啟後恢復。從舊版 `codex-monitor` 服務升級時，先執行 `docker compose down --remove-orphans` 停止舊容器，再執行 `docker compose up -d --build`，避免兩個服務同時更新 Discord 訊息。
